...
BOOST_AUTO_TEST_CASE(LFP_PointSource_LineSource) {
    ...
    std::array<double, 3> segment_end =
          segment_start + std::array<double,3>{0.0, 0.0, segment_length};
    double floor{1.0e-6};
    ...
    for (auto k = 0; k < 10; k++) {
        std::array<double, 3> approaching_elec = segment_middle + 
               std::array<double,3>{0.0, 1.0e-5 - k * 1.0e-6, 0.0};
        std::array<double, 3> circling_elec = segment_middle + 
               std::array<double,3>{0.0, circling_radius * std::cos(2.0 * pi * k / 10),
        ...
        std::clog << "ANALYTIC line source " << analytic_circling_lfp << " vs NUMERIC line source LFP "
                  << numeric_circling_lfp << "\n";
