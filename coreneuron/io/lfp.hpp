...
template <typename Point3D, typename F> F norm(const Point3D &p1) {
...
Array3<F> axp(const Point3D &p1, const F alpha, const Point3D &p2) {
  return { p1[0] + alpha * p2[0], p1[1] + alpha * p2[1],
           p1[2] + alpha * p2[2] };
}
 ...
 */
template <typename Point3D, typename F>
F point_source_lfp_factor(const Point3D &e_pos, const Point3D &seg_pos,
                          const F radius, const F f) {
 ...
 */
template <typename Point3D, typename F>
F line_source_lfp_factor(const Point3D &e_pos, const Point3D &seg_0,
                         const Point3D &seg_1, const F radius, const F f) {
  nrn_assert(radius >= 0.0);
    ...
    if (std::abs(q2) < std::numeric_limits<F>::epsilon()) {
      if (a * b <= 0) {
        throw std::invalid_argument(
            "Log integral: invalid arguments " + std::to_string(b) + " " +
            std::to_string(a) + ". Likely electrode exactly on the segment and "
...
struct LFPCalculator {
  ...
  LFPCalculator(const Point3Ds &seg_start, const Point3Ds &seg_end,
                const Vector &radius,
                const std::vector<SegmentIdTy> &segment_ids,
                const Point3Ds &electrodes, double extra_cellular_conductivity)
    ...
    double f(1.0 / (extra_cellular_conductivity * 4.0 *
  ...
  template <typename Vector> void lfp(const Vector &membrane_current) {
...
private:
  template <typename Point3D, typename F>
  inline F getFactor(const Point3D &e_pos, const Point3D &seg_0,
                     const Point3D &seg_1, const F radius, const F f) const;
...
F LFPCalculator<LineSource>::getFactor(const Point3D &e_pos,
                                       const Point3D &seg_0,
                                       const Point3D &seg_1, const F radius,
                                       const F f) const {
...
F LFPCalculator<PointSource>::getFactor(const Point3D &e_pos,
                                        ...
                                        const Point3D &seg_1, const F radius,
                                        const F f) const {
  return point_source_lfp_factor(e_pos, barycenter<Point3D, F>(seg_0, seg_1),
                                 radius, f);
}
};
